 ...
 */
class countriesBaseView extends AgaviView
{
	...
	public function setupHtml(AgaviRequestDataHolder $rd, $layoutName = null)
	{
		...
		if (!$this->getContainer()->getParameter('is_slot', false))
		{
			...
			if ($us->hasAttribute('message'))
			{
				$this->setAttribute('session_message', $us->getAttribute('message'));
				$us->removeAttribute('message');
			}
		}
	}
}
